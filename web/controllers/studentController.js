...
//Get students of group from database
    ...
    //Authorize
    $http.post(url + "/authorization/" +  $cookies.get('username')+"/"+$scope.group_id ).then(function successCallback(response) {
    ...
    //Get due Date
    $http.post(url + "/dueDateStudent/" + $scope.group_id).then(function successCallback(response) {
        if (response.data.success) {
            $scope.eval_due_date = response.data.info.due_date;
            $scope.server_date = response.data.server_date;
            $scope.form_disable = $scope.server_date > $scope.eval_due_date;
            console.log($scope.server_date);
        }
...
//Get questions of group from database
    ...
    //Authorize
    $http.post(url + "/authorization2/" +  $cookies.get('username')+"/"+$scope.group_id +"/"+ $scope.student_id).then(function successCallback(response) {
        console.log(JSON.stringify(response));
        if (response.data.success) {
            $scope.authorize = true;
            $scope.getQuestions();
